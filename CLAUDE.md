...
- 2026-09-15 — Added an admin-only `/expenses` section (break-even tracker): `coaches.monthly_salary`, plus new `branch_finance_settings` (per-branch rent type/value + a standard fee-per-student used only for projections) and `expenses` (one-off equipment/misc ledger) tables, both RLS-scoped to `is_admin()`. Break-even math lives in `lib/finance.ts` (unit-tested in `lib/__tests__/finance.test.ts`) rather than inline in the page, since it's the one piece of new business logic worth testing in isolation. Deliberately phase-1 only: current-month figures, no trend/forecast projections and no dashboard-level net-profit card yet — those need real historical expense data to be meaningful. RLS SQL was handed to the user to run manually (same pattern as the Manager rollout above) — not verified against the live Supabase project from this repo.
- 2026-09-16 — **Supersedes the schema in the entry above.** `coaches.monthly_salary` and `branch_finance_settings` were flat "current value" fields, which meant a salary raise or rent change would silently rewrite past months' figures when viewed later (caught by the user before it caused real confusion). Replaced both with effective-dated history tables — `coach_salary_history` and `branch_finance_revisions` (each row has an `effective_from: YYYY-MM`, and the value that applies to a given month is "the latest revision at or before that month," resolved via `resolveAsOf()` in `lib/finance.ts`). Also added `coaches.deactivated_at` (paired with the existing `created_at`) so a past month's coach-salary total reflects who was actually employed then, not today's active-coach list — resolved via `wasEmployedDuringMonth()`, also in `lib/finance.ts`. Deliberately does not reconstruct multiple past hire/rehire cycles, just one continuous employment span per coach. The equipment/misc `expenses` ledger was already correct (each row is its own dated fact) and didn't need this change.
- 2026-09-23 — Added a Dashboard-level "Net Profit" `StatCard` and a `/expenses/forecast` tab (Next Month / This Year / Next 3 Years horizons, admin-editable growth assumptions persisted in a new `forecast_assumptions` table). Extracted `lib/orgFinance.ts#fetchBranchMonthFinances()` as the shared per-branch/per-month data assembly (history-aware via `resolveAsOf`/`wasEmployedDuringMonth`), reused by both the Dashboard card and the forecast baseline — avoids a third reimplementation of that resolution logic. Projection math (compounding growth, break-even detection) lives in `lib/forecast.ts`, unit-tested like `lib/finance.ts`. Forecasting is gated behind >=3 real tracked months per branch (`hasEnoughHistory()`) to avoid showing a trend line off near-zero data. Deliberately scoped to continuous growth-rate assumptions only — no discrete one-off event modeling (e.g. "hire a coach in month 4") in this pass.
- 2026-09-25 — Fixed a real inconsistency the user caught: `components/admin/DashboardClient.tsx`'s Revenue/Pending Dues (pre-dating this file's tracked changes) filtered `fees` by `paid_date`/`created_at` falling within the picked period, while the Fees page, Expenses page, and the Net Profit card above all filter by `fees.month` (the billing month). A fee paid in a different month than it was billed for landed in different "months" depending on which page you looked at — most visibly, Net Profit ≠ Revenue − Total Expenses on the Dashboard despite Total Expenses correctly showing ₹0. Standardized the Dashboard's Revenue/Pending Dues/Collection Rate to the same `fees.month`-based filtering as everything else — one revenue definition app-wide. Also fixed `app/(admin)/fees/page.tsx`'s Paid/Unpaid Player *count* cards (not the ₹ amounts, which correctly still include everyone ever billed) to only count currently-active players, so they stay comparable to "Total Active Players" instead of including players who've since gone inactive/dropped but still have a fee row for that month.
- 2026-09-25 — Removed `fees.status = 'overdue'` entirely (schema + every reference) — grepped first and confirmed nothing in the codebase ever programmatically set it (no due-date-based auto-transition existed); the only write path was a manual dropdown option in `EditFeeModal.tsx` that the admin confirmed isn't used. `fees.status` is now just `'paid' | 'pending'` everywhere (`types/*.types.ts`, `FeeTable.tsx`, `EditFeeModal.tsx`, the Fees page's stats/filter tabs, and the three `.in('status', ['pending','overdue'])` call sites in `DashboardClient.tsx`, `coach-dashboard/page.tsx`, and `DeletePlayerModal.tsx`, all narrowed to `.eq('status', 'pending')`). The SQL handed to the user reassigns any existing `'overdue'` rows to `'pending'` before tightening the check constraint, so it's safe to run even if some were set manually in the past.
- 2026-09-26 — Added an optional absence reason to `attendance` (`absence_reason_category` — fixed list incl. `other` — plus a free-text `absence_reason_note`, both nullable) for retention/marketing insight, not reporting yet (deliberately deferred, same reasoning as gating Forecasting behind real history). Neither `app/(admin)/attendance/page.tsx` nor `app/(coach)/my-attendance/page.tsx` had (or has) a server-action file for this table — both write directly to Supabase from the client — so the reason fields were added to those same existing direct writes rather than introducing a new `actions.ts` just for this. Built one small shared, purely-controlled component (`components/shared/AbsenceReasonFields.tsx`) for the category-select + note-textarea to avoid duplicating that markup across the two otherwise-independent page implementations; each page still owns its own state and save timing exactly as before. A reason is only editable while a row is actively editable (today's unsubmitted marking, or the admin's edit mode) and is cleared automatically the moment a row flips back to `present`, so a present record can never carry a stale reason.
